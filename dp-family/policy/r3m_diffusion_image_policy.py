...
from model.vision.r3m_timm_obs_encoder import TimmObsEncoder
...
class DiffusionImagePolicy(BasePolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        batch_size, _obs_steps = value.shape[:2]
        ...
        if self.obs_as_global_cond:
            ...
            cond_data = torch.zeros(size=(batch_size, horizon, action_dim + obs_dim), device=device, dtype=dtype)
            cond_mask = torch.zeros_like(cond_data, dtype=torch.bool)
