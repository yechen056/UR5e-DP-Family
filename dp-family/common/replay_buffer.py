    ...
    def update_meta(self, data):
        ...
        if self.backend == 'zarr':
            for key, value in np_data.items():
                chunks = value.shape
                if value.shape:
                    # Zarr cannot create arrays with zero-sized chunks, which happens
                    # when metadata is truncated to an empty 1D array after dropping
                    # the last episode.
                    chunks = tuple(max(1, dim) for dim in value.shape)
                _ = meta_group.array(
                    name=key,
                    data=value, 
                    shape=value.shape, 
