...
class JointTrajectoryInterpolator:
    def __init__(self, times: np.ndarray, qpos: np.ndarray):
        """Initialize a linear joint interpolator

        Args:
            times (np.ndarray): (t,) array of times
            qpos (np.ndarray): (t, d) array of qpos
        """
        assert len(times) >= 1
        assert len(qpos) == len(times)
        if not isinstance(times, np.ndarray):
            times = np.array(times)
        if not isinstance(qpos, np.ndarray):
            qpos = np.array(qpos)

        self.qpos_dim = qpos[0].shape[0]
        if len(times) == 1:
            # special treatment for single step interpolation
            self.single_step = True
            self._times = times
            ...
            self.qpos_interp = si.interp1d(times, qpos, 
                axis=0, assume_sorted=True)
    ...
    def trim(self, 
        ...
        all_times = np.unique(all_times)
        # interpolate
        all_qpos = self(all_times)
        return JointTrajectoryInterpolator(times=all_times, qpos=all_qpos)
    
    def drive_to_waypoint(self, 
            qpos, time, curr_time,
            max_delta_qpos=np.inf,
        ) -> "JointTrajectoryInterpolator":
        assert(max_delta_qpos > 0)
        time = max(time, curr_time)
        
        curr_qpos = self(curr_time)
        qpos_dist = np.linalg.norm(qpos - curr_qpos)
        qpos_min_duration = qpos_dist / max_delta_qpos
        duration = time - curr_time
        duration = max(duration, qpos_min_duration)
        assert duration >= 0
        last_waypoint_time = curr_time + duration
        ...
        trimmed_interp = self.trim(curr_time, curr_time)
        times = np.append(trimmed_interp.times, [last_waypoint_time], axis=0)
        qpos = np.append(trimmed_interp.qpos, [qpos], axis=0)

        # create new interpolator
        final_interp = JointTrajectoryInterpolator(times=times, qpos=qpos)
        return final_interp

    def schedule_waypoint(self,
            qpos, time, 
            max_qpos_speed=np.inf,
            curr_time=None,
            last_waypoint_time=None
        ) -> "JointTrajectoryInterpolator":
        """Schedule a waypoint at time with qpos

        Args:
            qpos (np.ndarray): (d,) array of qpos
            time (float): time to schedule waypoint
            max_qpos_speed (float, optional): maximum speed of the qpos. Defaults to np.inf.
        ...
        Returns:
            JointTrajectoryInterpolator: new interpolator with scheduled waypoint
        """
        assert(max_qpos_speed > 0)
        if last_waypoint_time is not None:
            assert curr_time is not None
        assert qpos.shape[0] == self.qpos_dim
        ...
        # determine speed
        duration = time - end_time
        end_qpos = trimmed_interp(end_time)
        qpos_dist = np.linalg.norm(qpos - end_qpos)
        qpos_min_duration = qpos_dist / max_qpos_speed
        duration = max(duration, qpos_min_duration)
        assert duration >= 0
        last_waypoint_time = end_time + duration
