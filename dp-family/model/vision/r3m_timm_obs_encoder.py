...
class TimmObsEncoder(ModuleAttrMixin):
    def __init__(
        ...
        if model_name == "r3m":
            ...
            model = load_r3m("resnet18", pretrained=pretrained)
            model.eval()
        ...
        for key, attr in obs_shape_meta.items():
            ...
            elif obs_type in ("low_dim", "robot_state"):
        ...
        if model_name.startswith("vit"):
            if self.feature_aggregation == "all_tokens":
                ...
                logger.warning(
                    "vit will use the CLS token. feature_aggregation (%s) is ignored!",
    ...
    def forward(self, obs_dict):
        ...
        for key in self.rgb_keys:
            ...
            if img.shape[2:] != self.key_shape_map[key]:
                target_h = self.key_shape_map[key][1]
                target_w = self.key_shape_map[key][2]
